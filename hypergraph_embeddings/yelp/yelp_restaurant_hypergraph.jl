...
path_to_yelp = homedir()*"/log-pagerank/hypergraph_embeddings/data/yelp_dataset/"
...
function load_data_from_mat()
    ...
    # Business metadata
    Bus = matread(path_to_yelp*"yelp-business-metadata.mat")
    C = Bus["C"]
    ...
    n = M["n"]
    m = M["m"]
    return user_avgstar, user_elite, user_int2id, user_revcount,
            C, business_locations, city_int2name, zipcode_int2name, 
            state_int2name, cat_int2name, business_name,
            business_latlong, business_revcount,
            B, U, n, m
end

# Load
user_avgstar, user_elite, user_int2id, user_revcount,
C, business_locations, city_int2name, zipcode_int2name, 
state_int2name, cat_int2name, business_name,
business_latlong, business_revcount,
B, U, n, m = load_data_from_mat()
