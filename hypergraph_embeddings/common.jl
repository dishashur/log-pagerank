...
function tl_cut(H::SparseMatrixCSC, S::Vector{Int64}, delta::Float64,order::Vector{Int64})
...
function read_dataset()
    ...
    max_size=10000
    clusters = Dict()
    M = matread("/p/mnt/homes/dshur/hgembed/TinyZoo.mat")
    NodeLabels = vec(M["NodeLabels"])
    LabelNames = M["NodeNames"] #LabelNames for amazon.mat
    H = M["H"]
    @show size(H)
    G = LH.graph(H,1.0)
    volA = sum(G.deg)
